...
public class DrawMap extends JFrame {
	...
	private char[][] map;
	private BufferedImage[] imgs; // Image files used to display house blocks
	...
	private Color[] lineColors = { Color.blue, Color.cyan, Color.darkGray,
			Color.magenta, Color.orange, Color.green, brown, Color.lightGray,
	...
	public DrawMap(String mapFile) {
		...
		try {
			...
			if (mapLength > 15)
				setSize((2 * mapWidth - 1) * gridSize + 10, (2 * mapLength + 2)
						* gridSize);
			else if (mapLength >= 10)
				setSize((2 * mapWidth - 1) * gridSize + 10, (2 * mapLength + 1)
						* gridSize);
			else
				setSize((2 * mapWidth - 1) * gridSize + 10, (2 * mapLength)
						* gridSize);
	...
	private Color roadColor(char c) {
		...
		i = 0;
		while ((i < assignedColors) && (charColors[i] != c))
			++i;
		if (i == assignedColors) {
			if (assignedColors == numLines)
				return lineColors[0];
	...
	public void drawRoads() {
		...
		display.fillRect(0, 0, (2 * mapWidth - 1) * gridSize,
				(2 * mapLength - 1) * gridSize);
		display.setColor(backColor);
		display.drawRect(0, 0, (2 * mapWidth - 1) * gridSize,
				(2 * mapLength - 1) * gridSize);
		display.drawRect(1, 1, (2 * mapWidth - 1) * gridSize - 2,
				(2 * mapLength - 1) * gridSize - 2);
		display.drawRect(2, 2, (2 * mapWidth - 1) * gridSize - 4,
				(2 * mapLength - 1) * gridSize - 4);
		display.drawRect(3, 3, (2 * mapWidth - 1) * gridSize - 6,
				(2 * mapLength - 1) * gridSize - 6);
			...
			if ((row % 2) == 0) { // Horizontal street
				for (i = 0; i < (2 * mapWidth - 1); ++i)
					...
					case ' ':
						...
						break;
					case '+':
						drawIntersection(i, row);
						break;
					default:
						color = roadColor(map[row][i]);
						drawHorRoad(i, row, color);
						break;
					}
			...
			while (((y1 == y2) && ((x1 + i) < x2))
				...
				if (y1 == y2) // Horizontal line
					if (x1 + i + dashSize < x2)
						...
						display.drawLine(x1 + i, y1, x2, y2);
				else if (y1 + i + dashSize < y2)
					display.drawLine(x1, y1 + i, x2, y1 + i + dashSize);
				else
					display.drawLine(x1, y1 + i, x2, y2);
				i = i + 2 * dashSize;
			}
		}
	}
	...
	private void drawHorRoad(int x, int y, Color c) {
		/* ================================================================= */
		display.setColor(c);
		display.fillRect(x * gridSize - gridSize / 5, y * gridSize + gridSize
				/ 5, gridSize + 2 * gridSize / 5, gridSize - 2 * gridSize / 5);
		dashedLine(x * gridSize - gridSize / 5, y * gridSize + gridSize / 2, x
				* gridSize + gridSize + gridSize / 5, y * gridSize + gridSize
				/ 2, white, 5);
	}
	...
	private void drawVerRoad(int x, int y, Color c) {
		...
		display.fillRect(x * gridSize + gridSize / 5, y * gridSize - gridSize
				/ 5, gridSize - 2 * gridSize / 5, gridSize + 2 * gridSize / 5);
		dashedLine(x * gridSize + gridSize / 2, y * gridSize - gridSize / 5, x
				* gridSize + gridSize / 2, y * gridSize + gridSize + gridSize
				/ 5, white, 5);
	}
	...
	private void drawBlock(int x, int y) {
		...
		if (imageFiles) {
			...
			display.drawImage(imgs[i], x * gridSize, y * gridSize, x * gridSize
					+ gridSize, y * gridSize + gridSize, 1, 1,
	...
	private void drawIntersection(int x, int y) {
		...
		if ((x > 0) && (map[y][x - 1] != ' '))
			...
			x1 = basex + gridSize / 2;
		if ((x < (2 * mapWidth - 2)) && (map[y][x + 1] != ' '))
			x2 = basex + 4 * gridSize / 5;
		else
			x2 = basex + gridSize / 2;
		if ((y > 0) && (map[y - 1][x] != ' '))
			y1 = basey + gridSize / 5;
			...
			y1 = basey + gridSize / 2;
		if ((y < (2 * mapLength - 2)) && (map[y + 1][x] != ' '))
			y2 = basey + 4 * gridSize / 5;
		else
			y2 = basey + gridSize / 2;

		if ((y > 0) && (y < (2 * mapLength - 2))
				&& (map[y - 1][x] == map[y + 1][x]) && (map[y - 1][x] != ' '))
			c = roadColor(map[y - 1][x]);
		else if ((x > 0) && (x < (2 * mapWidth - 2))
				&& (map[y][x - 1] == map[y][x + 1]) && (map[y][x - 1] != ' '))
			c = roadColor(map[y][x - 1]);
		else if ((y > 0)
				&& (map[y - 1][x] != ' ')
				&& (((x > 0) && (map[y][x - 1] == map[y - 1][x])) || ((x < (2 * mapWidth - 2)) && (map[y][x + 1] == map[y - 1][x]))))
			c = roadColor(map[y - 1][x]);
		else if ((y < (2 * mapLength - 2))
				&& (map[y + 1][x] != ' ')
				&& (((x > 0) && (map[y][x - 1] == map[y + 1][x])) || ((x < (2 * mapWidth - 2)) && (map[y][x + 1] == map[y + 1][x]))))
			c = roadColor(map[y + 1][x]);
		else if ((y > 0) && (map[y - 1][x] != ' '))
			c = roadColor(map[y - 1][x]);
		else if ((y < (2 * mapLength - 2)) && (map[y + 1][x] != ' '))
			c = roadColor(map[y + 1][x]);
		else if ((x > 0) && (map[y][x - 1] != ' '))
			c = roadColor(map[y][x - 1]);
		else if ((x < (2 * mapWidth - 2)) && (map[y][x + 1] != ' '))
			c = roadColor(map[y][x + 1]);
		else
			c = streetColor;
	...
	private void drawBus(int x, int y) {
		...
		display.fillRoundRect(basex + 7 * gridSize / 16, basey + 1,
				gridSize / 16, gridSize / 4 - 2, 2, 2);

		dashedLine(basex + 2, basey, basex + 7 * gridSize / 16 - 2, basey,
				Color.gray, windowsize);
		dashedLine(basex + 2, basey + gridSize / 4 - 2, basex + 7 * gridSize
				/ 16 - 2, basey + gridSize / 4 - 2, Color.gray, windowsize);
		dashedLine(basex + 2, basey + 1, basex + 7 * gridSize / 16 - 2,
				basey + 1, Color.gray, windowsize);
		dashedLine(basex + 2, basey + gridSize / 4 - 1, basex + 7 * gridSize
				/ 16 - 2, basey + gridSize / 4 - 1, Color.gray, windowsize);

		display.setColor(Color.orange);
		display.fillRoundRect(basex + gridSize / 2 - 3, basey + 1, 3,
				gridSize / 8 - 2, 2, 2);
		display.fillRoundRect(basex + gridSize / 2 - 3, basey + gridSize / 8
				+ 1, 3, gridSize / 8 - 2, 2, 2);
	}
	...
	private void drawStart(int x, int y, boolean bus) {
		...
		for (int i = 0; i < 4; ++i)
			dashedLine(basex + 3 * gridSize / 5 + i, basey + gridSize / 5,
					basex + 3 * gridSize / 5 + i, basey + 4 * gridSize / 5,
					Color.yellow, 5);
	...
	private void drawExit(int x, int y) {
		...
		display.fillOval(basex + gridSize / 4, basey + gridSize / 4,
				gridSize / 2, gridSize / 2);
		display.setColor(white);
		display.fillOval(basex + gridSize / 3, basey + gridSize / 3,
				gridSize / 3, gridSize / 3);
		display.setColor(Color.red);
		display.fillOval(basex + 5 * gridSize / 12, basey + 5 * gridSize / 12,
